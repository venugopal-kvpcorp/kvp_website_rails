...
module Refinery
  class Page < Core::BaseModel
    ...
    # meta_keywords field directly to page instead of SEO Meta modal.
    ...
    attr_accessible :id, :deletable, :link_url, :menu_match,
                    ...
                    :layout_template, :view_template, :custom_slug, :slug, :canonical_url, :hreflang_url, :sitemap_priority, :allow_repayment_widget, :minimum_value, :maximum_value, :default_value,
                    :path, :title, :show_in_navigation, :display_link_text, :priority, :show_in_sitemap,
                    *::SeoMeta.attributes.keys, :meta_keywords, :image_id

    # Defining an association for FAQ
    has_many :question_answers, :class_name => "Refinery::Qas::Qa"  

    validates :title, :presence => true

    validates :custom_slug, :uniqueness => true, :allow_blank => true

    validates :display_link_text, :presence => true, :if => "show_in_navigation?"
    ...
    before_validation do |page|
      page.display_link_text = page.title if (page.show_in_navigation && page.display_link_text.nil?)
    end
    ...
    class << self
      ...
      def in_sitemap
        array_sitemap = where(:show_in_sitemap => true).with_globalize.select{|v| v.display_link_text != "Home"}
        swap_arr =  array_sitemap[3]
        array_sitemap[3] = array_sitemap[4]
        array_sitemap[4] = swap_arr
        array_sitemap
      end
    ...
    def load_with_parts(layout_template_name, force_save = false)
      self.layout_template = layout_template_name
      parts << Refinery::PagePart.for_layout_template(layout_template_name).map(&:dup)
      save! if force_save
    end
    ...
    def self.base_pages
      Refinery::Page.where(:parent_id => nil, :show_in_navigation => true).order("lft")
    end

    def self.child_pages_for(page)
      return if page.nil?
      where(:parent_id => page.id, :show_in_navigation => true).order("lft")
    end

    # Method to return all the FAQ pages
    def self.get_faq_pages
  ...
  private
    ...
    def normalize_friendly_id_with_marketable_urls(slug_string)
      ...
      if !Pages.scope_slug_by_parent && slug_string.include?('/')
        ...
        if Pages.marketable_urls && ( self.class.friendly_id_config.reserved_words - page_path_white_list ).include?(sluggified)
          sluggified << "-page"
        end
        sluggified
      end
    ...
    def page_path_white_list
      [ "investors", "complete", "media", "interested", "ebook-interested", "press-releases"]
    end
  end
end
