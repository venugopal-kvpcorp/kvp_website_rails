  ...
  module Admin
    class ImagesController < ::Refinery::AdminController
        ...
        begin
          if params[:image].present? && params[:image][:image].is_a?(Array)
            params[:image][:image].each do |image|
              @image = ::Refinery::Image.create({:image => image}.merge(image_params.except(:image)))
              @image.alternate_text = image_params[:image][:alternate_text]
              @image.save
              @images << @image
            end
          else
            @image = ::Refinery::Image.create(image_params)
            @image.alternate_text = image_params[:image][:alternate_text]
            @image.save
            @images << @image
          end
      ...
      def update
        ...
        if image_params[:image][:image].present?
          @image.alternate_text = image_params[:image][:alternate_text]
          @image.attributes = get_image_params
        else
          @image.alternate_text = image_params[:image][:alternate_text]
        end
      ...
      def image_params
        params.slice(:image, :image_size, :alternate_text)
      end

      def get_image_params
        image_params[:image].delete(:alternate_text)
        image_params[:image]
      end
